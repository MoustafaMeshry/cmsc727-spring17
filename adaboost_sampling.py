...
def printResultsSummary(resultsArr, header=None, metric="Results", entryLabel="iter", fout=sys.stdout):
    if header:
        fout.write(header + "\n");
...
if (not os.path.isfile(metaDataFile)):
    ...
    metaData[adaScoresTrainKey] = np.zeros(len(trainLabels) * 2).reshape(len(trainLabels), 2); # 2 = num of classes
    metaData[adaScoresTestKey] = np.zeros(len(testLabels) * 2).reshape(len(testLabels), 2); # 2 = num of classes
else:
